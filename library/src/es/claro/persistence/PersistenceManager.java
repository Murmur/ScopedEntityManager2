...
public class PersistenceManager {
	 ...
	 * Instance is owned by a current thread id.
	 * Use "default" instance name. 
	 * @return	JPA entity manager, new instance on first call then return the same instance.
	 */
	 ...
	 * 	    use "Thread.currentThread().getId()" ownerId for an automanaged instance.
		...
		for(Long key : emList.keySet()) {			
			...
			if (list!=null) {
				for(int idxb=0; idxb<list.size(); idxb++) {
					...
					stats.put("em"+idx+"."+idxb+".numOfAttributes", ""+em.getAttributeCount() );
				}
			}
		}
